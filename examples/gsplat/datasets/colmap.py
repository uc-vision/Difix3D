...
class Parser:
  ...
  def __init__(self, data_dir: str, factor: int = 1, normalize: bool = False, test_every: int = 8):
    ...
    if "3dv-dataset-nerfstudio" in data_dir:
      colmap_files = sorted(
        _get_rel_paths(colmap_image_dir), key=lambda x: int(x.split(".")[0].split("_")[-1])
      )
      image_files = sorted(
        _get_rel_paths(image_dir), key=lambda x: int(x.split(".")[0].split("_")[-1])
      )
    ...
    for camera_id in self.params_dict.keys():
      ...
      if camtype == "perspective":
        ...
        mask = np.logical_and(
          np.logical_and(mapx > 0, mapy > 0), np.logical_and(mapx < width - 1, mapy < height - 1)
        )
