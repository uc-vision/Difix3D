...
from datasets.traj import generate_interpolated_path, generate_ellipse_path_z, generate_spiral_path
...
from lib_bilagrid import BilateralGrid, slice, color_correct, total_variation_loss
...
class Runner:
  ...
  def __init__(self, local_rank: int, world_rank, world_size: int, cfg: Config) -> None:
    ...
    self.trainset = Dataset(
      self.parser, split="train", patch_size=cfg.patch_size, load_depths=cfg.depth_loss
    )
    ...
    if cfg.app_opt:
      ...
      self.app_optimizers = [
        ...
        torch.optim.Adam(
          self.app_module.color_head.parameters(), lr=cfg.app_opt_lr * math.sqrt(cfg.batch_size)
        ),
      ]
      if world_size > 1:
    ...
    if cfg.use_bilateral_grid:
      ...
      self.bil_grid_optimizers = [
        torch.optim.Adam(
          self.bil_grids.parameters(), lr=2e-3 * math.sqrt(cfg.batch_size), eps=1e-15
        )
      ]
    ...
    if not self.cfg.disable_viewer:
      ...
      self.viewer = nerfview.Viewer(
        server=self.server, render_fn=self._viewer_render_fn, mode="training"
      )
  ...
  def rasterize_splats(
    ...
    **kwargs,
    ...
    render_colors, render_alphas, info = rasterization(
      ...
      absgrad=(
        self.cfg.strategy.absgrad if isinstance(self.cfg.strategy, DefaultStrategy) else False
      ),
  ...
  def train(self, step=0):
    ...
    for step in pbar:
      ...
      if cfg.depth_loss:
        ...
        points = torch.stack(
          [points[:, :, 0] / (width - 1) * 2 - 1, points[:, :, 1] / (height - 1) * 2 - 1], dim=-1
        )  # normalize to [-1, 1]
  ...
  def fix(self, step: int):
    ...
    if len(self.cfg.fix_steps) == 1:
      ...
      novel_poses = self.interpolator.shift_poses(
        self.current_novel_poses, self.parser.camtoworlds[self.valset.indices], distance=0.5
      )

    self.render_traj(step, novel_poses)
    image_paths = [
      f"{self.render_dir}/novel/{step}/Pred/{i:04d}.png" for i in range(len(novel_poses))
    ]
    ...
    if len(self.novelloaders) == 0:
      ...
      ref_image_paths = [
        self.parser.image_paths[i] for i in np.array(self.trainset.indices)[ref_image_indices]
      ]
    else:
      ref_image_indices = self.interpolator.find_nearest_assignments(
        self.parser.camtoworlds[self.trainset.indices], novel_poses
    ...
    parser.test_every = 0
    parser.image_paths = [
      f"{self.render_dir}/novel/{step}/Fixed/{i:04d}.png" for i in range(len(novel_poses))
    ]
    parser.image_names = [os.path.basename(p) for p in parser.image_paths]
    parser.alpha_mask_paths = [
      f"{self.render_dir}/novel/{step}/Alpha/{i:04d}.png" for i in range(len(novel_poses))
    ]
