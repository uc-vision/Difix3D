...
def main(args):
  ...
  net_difix = Difix(lora_rank_vae=args.lora_rank_vae, timestep=args.timestep, mv_unet=args.mv_unet)
  net_difix.set_train()
      ...
      with accelerator.accumulate(*l_acc):
        ...
        if args.lambda_gram > 0:
          if global_step > args.gram_loss_warmup_steps:
            ...
            loss_gram = (
              gram_loss(x_tgt_pred_renorm.to(weight_dtype), x_tgt_renorm.to(weight_dtype), net_vgg)
              * args.lambda_gram
            )
            loss += loss_gram
          else:
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "--num_samples_eval", type=int, default=100, help="Number of samples to use for all evaluation"
  )

  parser.add_argument(
    "--viz_freq", type=int, default=100, help="Frequency of visualizing the outputs."
  )
  parser.add_argument(
